...
namespace Library.Characters.Villains
{
    public abstract class AbstractVillain : AbstractCharacter
    {
        ...
        {
            if(!this.IsAlive() || !hero.IsAlive()) throw new CannotAttackDeadException("Uno de los dos caracteres que iba a ser atacado estaba muerto.");
            hero.Hp = Math.Max(0, hero.Hp - Math.Max(0, this.Damage - hero.Defense));
        }
    }
}